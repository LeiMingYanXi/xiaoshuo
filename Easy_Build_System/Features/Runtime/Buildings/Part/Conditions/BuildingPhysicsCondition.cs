...
namespace EasyBuildSystem.Features.Runtime.Buildings.Part.Conditions
{
    [BuildingCondition("Building Physics Condition",
        "This condition checks if the Building Part is stable, otherwise, gravity will be applied to it.\n\n" +
    ...
    public class BuildingPhysicsCondition : BuildingCondition
    {
        ...
        public bool CheckStability()
        {
            ...
            for (int i = 0; i < m_Points.Length; i++)
            {
                ...
                {
                    if (Application.isPlaying && m_ShowDebugs)
                    {
        ...
        {
            if (!Application.isPlaying || GetBuildingPart == null || GetBuildingPart.State != BuildingPart.StateType.PLACED || IsFalling)
            {
                return;
            }
            ...
            foreach (Collider collider in GetBuildingPart.Colliders)
            {
                if (collider != null)
                {
            ...
            GetBuildingPart.DisableChildrenSockets();
